...
public final class Bindings {
    ...
    // Function Bindings
    private final Map<Ast.Method, Environment.Variable> methodBindings = new HashMap<>();
    private final Map<Ast.Expression.Function, Environment.Function> functionBindings = new HashMap<>();

    public void setMethod(Ast.Method method, Environment.Variable variable) { methodBindings.put(method, variable); }
    public Environment.Variable getMethod(Ast.Method method) { return methodBindings.get(method); }
    ...
    public Environment.Type getType(Ast.Expression node, Environment.Type type) { return typeBindings.get(node); }
}
