 ...
 * type declarations for each field.  This lexed successfully as shown
 * in that file.
 * 
...
// source String representation of the test case
...
Note, there are two formats that follow
 (1) Token list generated by using System.out.println(tokens);
     generating the list of tokens using the toString representation
     of each token.
...
Ast.Source {
	fields = [
		Ast.Field { name = 'i', constant = false, value = Optional[Ast.Expression.Literal{literal=-1}] }
		Ast.Field { name = 'inc', constant = false, value = Optional[Ast.Expression.Literal{literal=2}] }
	]
...
public class FooTestData {
    ...
    public static final Ast.Source tree_main = new Ast.Source(
        Arrays.asList(
            new Ast.Field("i", false, Optional.of(new Ast.Expression.Literal(BigInteger.valueOf(-1)))),
            new Ast.Field("inc", false, Optional.of(new Ast.Expression.Literal(BigInteger.valueOf(2))))
        ),
